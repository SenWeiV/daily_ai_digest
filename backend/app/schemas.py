...
class GitHubDigestItem(BaseModel):
    ...
    stars_today: int = Field(default=0, description="兼容字段：Trending周期新增Star")
    forks: int = Field(default=0, description="Fork数")
    watchers: int = Field(default=0, description="Watcher数")
    open_issues: int = Field(default=0, description="开放Issue和PR数量")
    recent_issue_comments: Optional[int] = Field(default=None, description="近期Issue/PR评论数，None表示不可用")
    source_channel: str = Field(default="legacy", description="来源: trending/search/legacy")
    trending_rank: Optional[int] = Field(default=None, description="GitHub Trending排名")
    trending_period: Optional[str] = Field(default=None, description="Trending周期: daily/weekly/monthly")
    recent_stars: int = Field(default=0, description="Trending周期新增Star")
    recent_star_period_days: int = Field(default=1, description="Trending周期天数")
    recent_star_velocity: float = Field(default=0.0, description="日均近期新增Star")
